...
fn main() {
    ...
    if let Some(name) = option_env!("BUILTIN_BACKEND") {
        ...
        let dylib = sysroot.join("lib").join(
            env::consts::DLL_PREFIX.to_string() + "rustc_codegen_tpde" + env::consts::DLL_SUFFIX,
        );
    ...
    if args.get(0).map(|arg| &**arg) == Some("tpde") {
        // Avoid infinite recursion when invoking `cargo-tpde` as cargo subcommand using
        // `cargo tpde`.
        args.remove(0);
    }
